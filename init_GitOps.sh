...
function wait_for_resource() {
  ...
  local namespace=${2:-}
  printf "\n%bWaiting for %s %b\n" "${CYAN_BG}" "${resource}" "${NC}"
  until oc get "$resource" ${namespace:+-n $namespace}; do
    printf "."
    sleep $RECHECK_TIMER
  done
  printf "\n%b%s is ready%b\n" "${GREEN}" "$resource" "${NC}"
...
function deploy() {
  ...
  if check_op_status; then
    ...
    oc adm new-project openshift-gitops-operator || true
    printf "\n"
    $HELM_BIN template --set 'helper-operator.enabled=true' --set 'helper-status-checker.enabled=true' --set 'gitopsinstances.openshift_gitops.clusterAdmin=disabled' -f values-openshift-gitops.yaml tjungbauer/openshift-gitops | oc create -f -

    printf "\nGive the gitops-operator some time to be installed. %bWaiting for %s seconds...%b\n" "${RED}" "${TIMER}" "${NC}"
    TIMER_TMP=0
    while [[ $TIMER_TMP -le $TIMER ]]; do
      printf "."
      sleep 1
...
function configure_argocd() {
  ...
  $HELM_BIN template --set 'gitopsinstances.openshift_gitops.enabled=true' --set 'gitopsinstances.openshift_gitops.clusterAdmin=enabled' -f values-openshift-gitops.yaml tjungbauer/openshift-gitops | oc replace -f -
...
command -v $HELM_BIN || error "Could not execute helm binary!"
